...
const Device = ({ devices }: { devices: DeviceSchema[] }) => {
  ...
  useEffect(() => {
    const host = '2efecadacb8544af95a7cd4a873e2d87.s1.eu.hivemq.cloud';
    const port = 8884;
    ...
    mqttClient.on('connect', () => {
      ...
      // Đăng ký các topic esp32/alert và esp32/number
      mqttClient.subscribe('esp32/alert', { qos: 0 });
      mqttClient.subscribe('esp32/number', { qos: 0 });
    });
    ...
    mqttClient.on('message', (topic, message) => {
      ...
      if (topic === 'esp32/alert') {
        // Xử lý giá trị boolean từ topic esp32/alert
        const alertState = JSON.parse(msg).state;
        setAlert(alertState);
        // console.log('Alert State:', alertState);
      } else if (topic === 'esp32/number') {
        // Xử lý giá trị từ topic esp32/number
  ...
  const toggleDevice = async (deviceId: string, deviceName: string) => {
    if (client) {
      ...
      client.publish(`esp32/${deviceName}`, JSON.stringify({ state: newState }));
